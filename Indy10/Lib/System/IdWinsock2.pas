...
begin
  if hWinSockDll = 0 then begin
    ...
    raise EIdWinsockStubError.Build(Format(RSWinsockLoadError, [WINSOCK2_DLL]), LError);
  end;
end;
...
begin
  if hMSWSockDll = 0 then begin
    ...
    if hMSWSockDll = 0 then begin
      raise EIdWinsockStubError.Build(Format(RSWinsockLoadError, [MSWSOCK_DLL]), Windows.GetLastError);
    end;
  end;
end;
...
procedure FixupStubEx(hSocket: TSocket; const AName: string; const AGuid: TGUID; var VStub: Pointer);
var
  LStatus: Integer;
  LBytesSend: DWORD;
begin
  ...
  if LStatus <> 0 then begin
    EIdWinsockStubError.Build(Format(RSWinsockCallError, [AName]), WSAGetLastError);
  end;
end;
...
const
  GuidTransmitFile: TGUID = (D1:$b5367df0; D2:$cbac; D3:$11cf; D4:($95, $ca, $00, $80, $5f, $48, $a1, $92));
begin
  FixupStubEx(hSocket, 'TransmitFile', GuidTransmitFile, @TransmitFile);
...
This is because GetAcceptExSockaddrs() is not passed a SOCKET can can be passed to
...
function Stub_AcceptEx(sListenSocket, sAcceptSocket: TSocket;
  ...
  lpOverlapped: POverlapped): BOOL; stdcall;
const
  GuidAcceptEx: TGuid = (D1:$b5367df1;D2:$cbac;D3:$11cf;D4:($95,$ca,$00,$80,$5f,$48,$a1,$92));
  GuidGetAcceptExSockaddrs: TGuid = (D1:$b5367df2;D2:$cbac;D3:$11cf;D4:($95,$ca,$00,$80,$5f,$48,$a1,$92));
begin
  {RLebeau - loading GetAcceptExSockaddrs() first in case it fails}
  FixupStubEx(sListenSocket, 'GetAcceptExSockaddrs', GuidGetAcceptExSockaddrs, @GetAcceptExSockaddrs);
  FixupStubEx(sListenSocket, 'AcceptEx', GuidAcceptEx, @AcceptEx);
...
begin
  if cmsg = nil then begin
    ...
    if (DWORD(cmsg) + WSA_CMSGHDR_ALIGN(cmsg^.cmsg_len) + SizeOf(WSACMSGHDR)) > (DWORD(msg^.Control.buf) + msg^.Control.len) then begin
      Result := nil;
    end else begin
      Result := LPWSACMSGHDR(DWORD(cmsg) + WSA_CMSGHDR_ALIGN(cmsg^.cmsg_len));
    end;
  end;
end;
