...
REM ****************************************************************************
REM  C++Builder 5 build for Indy (packages relocated; source resolves via ..\..\Source).
REM  Build runs in the Output\C5 temp dir so the dpk's ..\..\Source paths resolve.
REM ****************************************************************************
...
copy ..\..\Source\Protocols\ZLib\*.obj ..\..\Output\C5 > nul
cd ..\..\Output\C5
...
%NDC5%\bin\dcc32.exe /B IndyProtocols50.dpk
if errorlevel 1 goto enderror
..\..\LspFix.exe IndyProtocols50.lsp
...
REM ----- relocate finished artifacts up to the repo-root Output\C5 build folder -----
if not exist ..\..\..\Output\C5\*.* md ..\..\..\Output\C5 >nul
move /Y *.* ..\..\..\Output\C5\ >nul
