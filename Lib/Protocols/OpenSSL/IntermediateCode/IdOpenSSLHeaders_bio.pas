...
var
  function BIO_get_new_index: TIdC_INT;
  procedure BIO_set_flags(b: PBIO; flags: TIdC_INT);
  function BIO_test_flags(const b: PBIO; flags: TIdC_INT): TIdC_INT;
  procedure BIO_clear_flags(b: PBIO; flags: TIdC_INT);

  function BIO_get_callback(b: PBIO): BIO_callback_fn;
  procedure BIO_set_callback(b: PBIO; callback: BIO_callback_fn);

  function BIO_get_callback_ex(b: PBIO): BIO_callback_fn_ex;
  procedure BIO_set_callback_ex(b: PBIO; callback: BIO_callback_fn_ex);

  function BIO_get_callback_arg(const b: PBIO): PIdAnsiChar;
  procedure BIO_set_callback_arg(var b: PBIO; arg: PIdAnsiChar);

  function BIO_method_name(const b: PBIO): PIdAnsiChar;
  function BIO_method_type(const b: PBIO): TIdC_INT;
...
//    CRYPTO_get_ex_new_index(CRYPTO_EX_INDEX_BIO, l, p, newf, dupf, freef)
  ...
  function BIO_set_ex_data(bio: PBIO; idx: TIdC_INT; data: Pointer): TIdC_INT;
  function BIO_get_ex_data(bio: PBIO; idx: TIdC_INT): Pointer;
  function BIO_number_read(bio: PBIO): TIdC_UINT64;
  function BIO_number_written(bio: PBIO): TIdC_UINT64;
...
//  function BIO_asn1_get_suffix(b: PBIO; psuffix: ^asn1_ps_func; psuffix_free: ^^asn1_ps_func): TIdC_INT;
  ...
  function BIO_new_file(const filename: PIdAnsiChar; const mode: PIdAnsiChar): PBIO;
//  function BIO_new_fp(stream: cFile; close_flag: TIdC_INT): PBIO;
  function BIO_new(const cType: PBIO_METHOD): PBIO;
  function BIO_free(a: PBIO): TIdC_INT;
  procedure BIO_set_data(a: PBIO; ptr: Pointer);
  function BIO_get_data(a: PBIO): Pointer;
  procedure BIO_set_init(a: PBIO; init: TIdC_INT);
  function BIO_get_init(a: PBIO): TIdC_INT;
  procedure BIO_set_shutdown(a: PBIO; shut: TIdC_INT);
  function BIO_get_shutdown(a: PBIO): TIdC_INT;
  ...
  function BIO_callback_ctrl(b: PBIO; cmd: TIdC_INT; fp: PBIO_info_cb): TIdC_LONG;
  function BIO_ptr_ctrl(bp: PBIO; cmd: TIdC_INT; larg: TIdC_LONG): Pointer;
  function BIO_int_ctrl(bp: PBIO; cmd: TIdC_INT; larg: TIdC_LONG; iarg: TIdC_INT): TIdC_LONG;
  function BIO_push(b: PBIO; append: PBIO): PBIO;
  function BIO_pop(b: PBIO): PBIO;
  procedure BIO_free_all(a: PBIO);
  function BIO_find_type(b: PBIO; bio_type: TIdC_INT): PBIO;
  function BIO_next(b: PBIO): PBIO;
  procedure BIO_set_next(b: PBIO; next: PBIO);
  ...
  function BIO_nwrite0(bio: PBIO; buf: PPIdAnsiChar): TIdC_INT;
  function BIO_nwrite(bio: PBIO; buf: PPIdAnsiChar; num: TIdC_INT): TIdC_INT;
...
//    buf: Pointer): TIdC_INT, Pointer context);
  ...
  function BIO_sock_should_retry(i: TIdC_INT): TIdC_INT;
...
//  function BIO_dump_indent_fp(fp: cFile; const s: PByte; len: TIdC_INT; indent: TIdC_INT): TIdC_INT;
  ...
  function BIO_ADDR_path_string(const ap: PBIO_ADDR): PIdAnsiChar;

  function BIO_ADDRINFO_next(const bai: PBIO_ADDRINFO): PBIO_ADDRINFO;
  function BIO_ADDRINFO_family(const bai: PBIO_ADDRINFO): TIdC_INT;
  function BIO_ADDRINFO_socktype(const bai: PBIO_ADDRINFO): TIdC_INT;
  function BIO_ADDRINFO_protocol(const bai: PBIO_ADDRINFO): TIdC_INT;
  function BIO_ADDRINFO_address(const bai: PBIO_ADDRINFO): PBIO_ADDR;
  procedure BIO_ADDRINFO_free(bai: PBIO_ADDRINFO);

  function BIO_parse_hostserv(const hostserv: PIdAnsiChar; host: PPIdAnsiChar; service: PPIdAnsiChar; hostserv_prio: BIO_hostserv_priorities): TIdC_INT;

  function BIO_lookup(const host: PIdAnsiChar; const service: PIdAnsiChar; lookup_type: BIO_lookup_type; family: TIdC_INT; socktype: TIdC_INT; res: PPBIO_ADDRINFO): TIdC_INT;
  function BIO_lookup_ex(const host: PIdAnsiChar; const service: PIdAnsiChar; lookup_type: TIdC_INT; family: TIdC_INT; socktype: TIdC_INT; protocol: TIdC_INT; res: PPBIO_ADDRINFO): TIdC_INT;
  function BIO_sock_error(sock: TIdC_INT): TIdC_INT;
  function BIO_socket_ioctl(fd: TIdC_INT; cType: TIdC_LONG; arg: Pointer): TIdC_INT;
  function BIO_socket_nbio(fd: TIdC_INT; mode: TIdC_INT): TIdC_INT;
  function BIO_sock_init: TIdC_INT;

  function BIO_set_tcp_ndelay(sock: TIdC_INT; turn_on: TIdC_INT): TIdC_INT;

  function BIO_sock_info(sock: TIdC_INT; type_: BIO_sock_info_type; info: PBIO_sock_info_u): TIdC_INT;

  function BIO_socket(domain: TIdC_INT; socktype: TIdC_INT; protocol: TIdC_INT; options: TIdC_INT): TIdC_INT;
  ...
  function BIO_accept_ex(accept_sock: TIdC_INT; addr: PBIO_ADDR; options: TIdC_INT): TIdC_INT;
  function BIO_closesocket(sock: TIdC_INT): TIdC_INT;

  function BIO_new_socket(sock: TIdC_INT; close_flag: TIdC_INT): PBIO;
  function BIO_new_connect(const host_port: PIdAnsiChar): PBIO;
  function BIO_new_accept(const host_port: PIdAnsiChar): PBIO;
