...
var
  ...
  function SSL_CTX_get_options(const ctx: PSSL_CTX): TIdC_ULONG;
  function SSL_get_options(const s: PSSL): TIdC_ULONG;
  function SSL_CTX_clear_options(ctx: PSSL_CTX; op: TIdC_ULONG): TIdC_ULONG;
  function SSL_clear_options(s: PSSL; op: TIdC_ULONG): TIdC_ULONG;
  function SSL_CTX_set_options(ctx: PSSL_CTX; op: TIdC_ULONG): TIdC_ULONG;
  function SSL_set_options(s: PSSL; op: TIdC_ULONG): TIdC_ULONG;
  ...
  procedure SSL_CTX_sess_set_new_cb(ctx: PSSL_CTX; new_session_cb: SSL_CTX_sess_new_cb);
  function SSL_CTX_sess_get_new_cb(ctx: PSSL_CTX): SSL_CTX_sess_new_cb;
  procedure SSL_CTX_sess_set_remove_cb(ctx: PSSL_CTX; remove_session_cb: SSL_CTX_sess_remove_cb);
  function SSL_CTX_sess_get_remove_cb(ctx: PSSL_CTX): SSL_CTX_sess_remove_cb;
  ...
  procedure SSL_CTX_set_info_callback(ctx: PSSL_CTX; cb: SSL_CTX_info_callback);
  function SSL_CTX_get_info_callback(ctx: PSSL_CTX): SSL_CTX_info_callback;
  procedure SSL_CTX_set_client_cert_cb(ctx: PSSL_CTX; client_cert_cb: SSL_CTX_client_cert_cb);
  function SSL_CTX_get_client_cert_cb(ctx: PSSL_CTX): SSL_CTX_client_cert_cb;
  function SSL_CTX_set_client_cert_engine(ctx: PSSL_CTX; e: PENGINE): TIdC_INT;

  procedure SSL_CTX_set_cookie_generate_cb(ctx: PSSL_CTX; app_gen_cookie_cb: SSL_CTX_cookie_verify_cb);
  procedure SSL_CTX_set_cookie_verify_cb(ctx: PSSL_CTX; app_verify_cookie_cb: SSL_CTX_set_cookie_verify_cb_app_verify_cookie_cb);
  procedure SSL_CTX_set_stateless_cookie_generate_cb(ctx: PSSL_CTX; gen_stateless_cookie_cb: SSL_CTX_set_stateless_cookie_generate_cb_gen_stateless_cookie_cb);
  procedure SSL_CTX_set_stateless_cookie_verify_cb(ctx: PSSL_CTX; verify_stateless_cookie_cb: SSL_CTX_set_stateless_cookie_verify_cb_verify_stateless_cookie_cb);
  ...
  procedure SSL_CTX_set_alpn_select_cb(ctx: PSSL_CTX; cb: SSL_CTX_alpn_select_cb_func; arg: Pointer);
  procedure SSL_get0_alpn_selected(const ssl: PSSL; const data: PPByte; len: PIdC_UINT);
  procedure SSL_CTX_set_psk_client_callback(ctx: PSSL_CTX; cb: SSL_psk_client_cb_func);
  procedure SSL_set_psk_client_callback(ssl: PSSL; cb: SSL_psk_client_cb_func);
  procedure SSL_CTX_set_psk_server_callback(ctx: PSSL_CTX; cb: SSL_psk_server_cb_func);
  ...
  procedure SSL_set_psk_find_session_callback(s: PSSL; cb: SSL_psk_find_session_cb_func);
  procedure SSL_CTX_set_psk_find_session_callback(ctx: PSSL_CTX; cb: SSL_psk_find_session_cb_func);
  procedure SSL_set_psk_use_session_callback(s: PSSL; cb: SSL_psk_use_session_cb_func);
  procedure SSL_CTX_set_psk_use_session_callback(ctx: PSSL_CTX; cb: SSL_psk_use_session_cb_func);
   ...
   *)
  procedure SSL_CTX_set_keylog_callback(ctx: PSSL_CTX; cb: SSL_CTX_keylog_cb_func);
  (*
   * SSL_CTX_get_keylog_callback returns the callback configured by
   * SSL_CTX_set_keylog_callback.
   *)
  function SSL_CTX_get_keylog_callback(const ctx: PSSL_CTX): SSL_CTX_keylog_cb_func;
  function SSL_CTX_set_max_early_data(ctx: PSSL_CTX; max_early_data: TIdC_UINT32): TIdC_INT;
  function SSL_CTX_get_max_early_data(const ctx: PSSL_CTX): TIdC_UINT32;
  function SSL_set_max_early_data(s: PSSL; max_early_data: TIdC_UINT32): TIdC_INT;
  function SSL_get_max_early_data(const s: PSSL): TIdC_UINT32;
  function SSL_CTX_set_recv_max_early_data(ctx: PSSL_CTX; recv_max_early_data: TIdC_UINT32): TIdC_INT;
  function SSL_CTX_get_recv_max_early_data(const ctx: PSSL_CTX): TIdC_UINT32;
  function SSL_set_recv_max_early_data(s: PSSL; recv_max_early_data: TIdC_UINT32): TIdC_INT;
  function SSL_get_recv_max_early_data(const s: PSSL): TIdC_UINT32;
  ...
  function SSL_in_init(const s: PSSL): TIdC_INT;
  function SSL_in_before(const s: PSSL): TIdC_INT;
  function SSL_is_init_finished(const s: PSSL): TIdC_INT;
   ...
   *)
  function SSL_get_finished(const s: PSSL; buf: Pointer; count: TIdC_SIZET): TIdC_SIZET;
  function SSL_get_peer_finished(const s: PSSL; buf: Pointer; count: TIdC_SIZET): TIdC_SIZET;
  ...
  //# endif
  //
  function BIO_f_ssl: PBIO_METHOD;
  ...
  function SSL_CTX_set_timeout(ctx: PSSL_CTX; t: TIdC_LONG): TIdC_LONG;
  function SSL_CTX_get_timeout(const ctx: PSSL_CTX): TIdC_LONG;
  function SSL_CTX_get_cert_store(const v1: PSSL_CTX): PX509_STORE;
  function SSL_want(const s: PSSL): TIdC_INT;
  function SSL_clear(s: PSSL): TIdC_INT;

  procedure BIO_ssl_shutdown(ssl_bio: PBIO);
  function SSL_CTX_up_ref(ctx: PSSL_CTX): TIdC_INT;
  procedure SSL_CTX_free(v1: PSSL_CTX);
  procedure SSL_CTX_set_cert_store(v1: PSSL_CTX; v2: PX509_STORE);
  procedure SSL_CTX_set1_cert_store(v1: PSSL_CTX; v2: PX509_STORE);

  procedure SSL_CTX_flush_sessions(ctx: PSSL_CTX; tm: TIdC_LONG);

  function SSL_get_current_cipher(const s: PSSL): PSSL_CIPHER;
  function SSL_get_pending_cipher(const s: PSSL): PSSL_CIPHER;
  function SSL_CIPHER_get_bits(const c: PSSL_CIPHER; alg_bits: PIdC_INT): TIdC_INT;
  function SSL_CIPHER_get_version(const c: PSSL_CIPHER): PIdAnsiChar;
  function SSL_CIPHER_get_name(const c: PSSL_CIPHER): PIdAnsiChar;
  function SSL_CIPHER_standard_name(const c: PSSL_CIPHER): PIdAnsiChar;
  function OPENSSL_cipher_name(const rfc_name: PIdAnsiChar): PIdAnsiChar;
  function SSL_CIPHER_get_id(const c: PSSL_CIPHER): TIdC_UINT32;
  function SSL_CIPHER_get_protocol_id(const c: PSSL_CIPHER): TIdC_UINT16;
  function SSL_CIPHER_get_kx_nid(const c: PSSL_CIPHER): TIdC_INT;
  function SSL_CIPHER_get_auth_nid(const c: PSSL_CIPHER): TIdC_INT;
  function SSL_CIPHER_get_handshake_digest(const c: PSSL_CIPHER): PEVP_MD;
  function SSL_CIPHER_is_aead(const c: PSSL_CIPHER): TIdC_INT;

  function SSL_get_fd(const s: PSSL): TIdC_INT;
  function SSL_get_rfd(const s: PSSL): TIdC_INT;
  function SSL_get_wfd(const s: PSSL): TIdC_INT;
  function SSL_get_cipher_list(const s: PSSL; n: TIdC_INT): PIdAnsiChar;
  function SSL_get_shared_ciphers(const s: PSSL; buf: PIdAnsiChar; size: TIdC_INT): PIdAnsiChar;
  function SSL_get_read_ahead(const s: PSSL): TIdC_INT;
  function SSL_pending(const s: PSSL): TIdC_INT;
  function SSL_has_pending(const s: PSSL): TIdC_INT;
  function SSL_set_fd(s: PSSL; fd: TIdC_INT): TIdC_INT;
  function SSL_set_rfd(s: PSSL; fd: TIdC_INT): TIdC_INT;
  function SSL_set_wfd(s: PSSL; fd: TIdC_INT): TIdC_INT;
  procedure SSL_set0_rbio(s: PSSL; rbio: PBIO);
  procedure SSL_set0_wbio(s: PSSL; wbio: PBIO);
  procedure SSL_set_bio(s: PSSL; rbio: PBIO; wbio: PBIO);
  function SSL_get_rbio(const s: PSSL): PBIO;
  function SSL_get_wbio(const s: PSSL): PBIO;
  function SSL_set_cipher_list(s: PSSL; const str: PIdAnsiChar): TIdC_INT;
  function SSL_CTX_set_ciphersuites(ctx: PSSL_CTX; const str: PIdAnsiChar): TIdC_INT;
  function SSL_set_ciphersuites(s: PSSL; const str: PIdAnsiChar): TIdC_INT;
  function SSL_get_verify_mode(const s: PSSL): TIdC_INT;
  function SSL_get_verify_depth(const s: PSSL): TIdC_INT;
  function SSL_get_verify_callback(const s: PSSL): SSL_verify_cb;
  procedure SSL_set_read_ahead(s: PSSL; yes: TIdC_INT);
  procedure SSL_set_verify(s: PSSL; mode: TIdC_INT; callback: SSL_verify_cb);
  procedure SSL_set_verify_depth(s: PSSL; depth: TIdC_INT);
  //void SSL_set_cert_cb(s: PSSL, TIdC_INT (*cb) (ssl: PSSL, void *arg), void *arg);

  function SSL_use_RSAPrivateKey(ssl: PSSL; rsa: PRSA): TIdC_INT;
  function SSL_use_RSAPrivateKey_ASN1(ssl: PSSL; const d: PByte; len: TIdC_LONG): TIdC_INT;
  function SSL_use_PrivateKey(ssl: PSSL; pkey: PEVP_PKEY): TIdC_INT;
  function SSL_use_PrivateKey_ASN1(pk: TIdC_INT; ssl: PSSL; const d: PByte; len: TIdC_LONG): TIdC_INT;
  function SSL_use_certificate(ssl: PSSL; x: PX509): TIdC_INT;
  function SSL_use_certificate_ASN1(ssl: PSSL; const d: PByte; len: TIdC_INT): TIdC_INT;
  ...
  function SSL_CTX_use_serverinfo(ctx: PSSL_CTX; const serverinfo: PByte; serverinfo_length: TIdC_SIZET): TIdC_INT;
  function SSL_CTX_use_serverinfo_ex(ctx: PSSL_CTX; version: TIdC_UINT; const serverinfo: PByte; serverinfo_length: TIdC_SIZET): TIdC_INT;
  function SSL_CTX_use_serverinfo_file(ctx: PSSL_CTX; const &file: PIdAnsiChar): TIdC_INT;

  function SSL_use_RSAPrivateKey_file(ssl: PSSL; const &file: PIdAnsiChar; &type: TIdC_INT): TIdC_INT;

  function SSL_use_PrivateKey_file(ssl: PSSL; const &file: PIdAnsiChar; &type: TIdC_INT): TIdC_INT;
  function SSL_use_certificate_file(ssl: PSSL; const &file: PIdAnsiChar; &type: TIdC_INT): TIdC_INT;

  function SSL_CTX_use_RSAPrivateKey_file(ctx: PSSL_CTX; const &file: PIdAnsiChar; &type: TIdC_INT): TIdC_INT;

  function SSL_CTX_use_PrivateKey_file(ctx: PSSL_CTX; const &file: PIdAnsiChar; &type: TIdC_INT): TIdC_INT;
  function SSL_CTX_use_certificate_file(ctx: PSSL_CTX; const &file: PIdAnsiChar; &type: TIdC_INT): TIdC_INT;
  (* PEM type *)
  function SSL_CTX_use_certificate_chain_file(ctx: PSSL_CTX; const &file: PIdAnsiChar): TIdC_INT;
  function SSL_use_certificate_chain_file(ssl: PSSL; const &file: PIdAnsiChar): TIdC_INT;
  ...
  function SSL_state_string(const s: PSSL): PIdAnsiChar;
  function SSL_rstate_string(const s: PSSL): PIdAnsiChar;
  function SSL_state_string_long(const s: PSSL): PIdAnsiChar;
  function SSL_rstate_string_long(const s: PSSL): PIdAnsiChar;
  function SSL_SESSION_get_time(const s: PSSL_SESSION): TIdC_LONG;
  function SSL_SESSION_set_time(s: PSSL_SESSION; t: TIdC_LONG): TIdC_LONG;
  function SSL_SESSION_get_timeout(const s: PSSL_SESSION): TIdC_LONG;
  function SSL_SESSION_set_timeout(s: PSSL_SESSION; t: TIdC_LONG): TIdC_LONG;
  function SSL_SESSION_get_protocol_version(const s: PSSL_SESSION): TIdC_INT;
  function SSL_SESSION_set_protocol_version(s: PSSL_SESSION; version: TIdC_INT): TIdC_INT;

  function SSL_SESSION_get0_hostname(const s: PSSL_SESSION): PIdAnsiChar;
  function SSL_SESSION_set1_hostname(s: PSSL_SESSION; const hostname: PIdAnsiChar): TIdC_INT;
  procedure SSL_SESSION_get0_alpn_selected(const s: PSSL_SESSION; const alpn: PPByte; len: PIdC_SIZET);
  function SSL_SESSION_set1_alpn_selected(s: PSSL_SESSION; const alpn: PByte; len: TIdC_SIZET): TIdC_INT;
  function SSL_SESSION_get0_cipher(const s: PSSL_SESSION): PSSL_CIPHER;
  function SSL_SESSION_set_cipher(s: PSSL_SESSION; const cipher: PSSL_CIPHER): TIdC_INT;
  function SSL_SESSION_has_ticket(const s: PSSL_SESSION): TIdC_INT;
  function SSL_SESSION_get_ticket_lifetime_hint(const s: PSSL_SESSION): TIdC_ULONG;
  ...
  function SSL_SESSION_is_resumable(const s: PSSL_SESSION): TIdC_INT;

  function SSL_SESSION_new: PSSL_SESSION;
  function SSL_SESSION_dup(src: PSSL_SESSION): PSSL_SESSION;
  function SSL_SESSION_get_id(const s: PSSL_SESSION; len: PIdC_UINT): PByte;
  function SSL_SESSION_get0_id_context(const s: PSSL_SESSION; len: PIdC_UINT): PByte;
  function SSL_SESSION_get_compress_id(const s: PSSL_SESSION): TIdC_UINT;
  function SSL_SESSION_print(fp: PBIO; const ses: PSSL_SESSION): TIdC_INT;
  ...
  function SSL_set_generate_session_id(s: PSSL; cb: GEN_SESSION_CB): TIdC_INT;
  function SSL_has_matching_session_id(const s: PSSL; const id: PByte; id_len: TIdC_UINT): TIdC_INT;
  ...
  //
  function SSL_CTX_get_verify_mode(const ctx: PSSL_CTX): TIdC_INT;
  function SSL_CTX_get_verify_depth(const ctx: PSSL_CTX): TIdC_INT;
  function SSL_CTX_get_verify_callback(const ctx: PSSL_CTX): SSL_verify_cb;
  procedure SSL_CTX_set_verify(ctx: PSSL_CTX; mode: TIdC_INT; callback: SSL_verify_cb);
  procedure SSL_CTX_set_verify_depth(ctx: PSSL_CTX; depth: TIdC_INT);
  procedure SSL_CTX_set_cert_verify_callback(ctx: PSSL_CTX; cb: SSL_CTX_set_cert_verify_callback_cb; arg: Pointer);
  procedure SSL_CTX_set_cert_cb(c: PSSL_CTX; cb: SSL_CTX_set_cert_cb_cb; arg: Pointer);
  function SSL_CTX_use_RSAPrivateKey(ctx: PSSL_CTX; rsa: PRSA): TIdC_INT;
  function SSL_CTX_use_RSAPrivateKey_ASN1(ctx: PSSL_CTX; const d: PByte; len: TIdC_LONG): TIdC_INT;
  function SSL_CTX_use_PrivateKey(ctx: PSSL_CTX; pkey: PEVP_PKEY): TIdC_INT;
  function SSL_CTX_use_PrivateKey_ASN1(pk: TIdC_INT; ctx: PSSL_CTX; const d: PByte; len: TIdC_LONG): TIdC_INT;
  function SSL_CTX_use_certificate(ctx: PSSL_CTX; x: X509): TIdC_INT;
  function SSL_CTX_use_certificate_ASN1(ctx: PSSL_CTX; len: TIdC_INT; const d: PByte): TIdC_INT;
  ...
  procedure SSL_CTX_set_default_passwd_cb(ctx: PSSL_CTX; cb: pem_password_cb);
  procedure SSL_CTX_set_default_passwd_cb_userdata(ctx: PSSL_CTX; u: Pointer);
  function SSL_CTX_get_default_passwd_cb(ctx: PSSL_CTX): pem_password_cb;
  function SSL_CTX_get_default_passwd_cb_userdata(ctx: PSSL_CTX): Pointer;
  procedure SSL_set_default_passwd_cb(s: PSSL; cb: pem_password_cb);
  procedure SSL_set_default_passwd_cb_userdata(s: PSSL; u: Pointer);
  function SSL_get_default_passwd_cb(s: PSSL): pem_password_cb;
  function SSL_get_default_passwd_cb_userdata(s: PSSL): Pointer;

  function SSL_CTX_check_private_key(const ctx: PSSL_CTX): TIdC_INT;
  function SSL_check_private_key(const ctx: PSSL): TIdC_INT;

  function SSL_CTX_set_session_id_context(ctx: PSSL_CTX; const sid_ctx: PByte; sid_ctx_len: TIdC_UINT): TIdC_INT;

  function SSL_new(ctx: PSSL_CTX): PSSL;
  function SSL_up_ref(s: PSSL): TIdC_INT;
  function SSL_is_dtls(const s: PSSL): TIdC_INT;
  function SSL_set_session_id_context(ssl: PSSL; const sid_ctx: PByte; sid_ctx_len: TIdC_UINT): TIdC_INT;

  function SSL_CTX_set_purpose(ctx: PSSL_CTX; purpose: TIdC_INT): TIdC_INT;
  ...
  function SSL_set1_host(s: PSSL; const hostname: PIdAnsiChar): TIdC_INT;
  function SSL_add1_host(s: PSSL; const hostname: PIdAnsiChar): TIdC_INT;
  function SSL_get0_peername(s: PSSL): PIdAnsiChar;
  procedure SSL_set_hostflags(s: PSSL; flags: TIdC_UINT);

  function SSL_CTX_dane_enable(ctx: PSSL_CTX): TIdC_INT;
  function SSL_CTX_dane_mtype_set(ctx: PSSL_CTX; const md: PEVP_MD; mtype: TIdC_UINT8; ord: TIdC_UINT8): TIdC_INT;
  function SSL_dane_enable(s: PSSL; const basedomain: PIdAnsiChar): TIdC_INT;
  function SSL_dane_tlsa_add(s: PSSL; usage: TIdC_UINT8; selector: TIdC_UINT8; mtype: TIdC_UINT8; const data: PByte; dlen: TIdC_SIZET): TIdC_INT;
  function SSL_get0_dane_authority(s: PSSL; mcert: PPX509; mspki: PPEVP_PKEY): TIdC_INT;
  function SSL_get0_dane_tlsa(s: PSSL; usage: PIdC_UINT8; selector: PIdC_UINT8; mtype: PIdC_UINT8; const data: PPByte; dlen: PIdC_SIZET): TIdC_INT;
  (*
   * Bridge opacity barrier between libcrypt and libssl, also needed to support
   * offline testing in test/danetest.c
   *)
  function SSL_get0_dane(ssl: PSSL): PSSL_DANE;

  (*
   * DANE flags
   *)
  function SSL_CTX_dane_set_flags(ctx: PSSL_CTX; flags: TIdC_ULONG): TIdC_ULONG;
  function SSL_CTX_dane_clear_flags(ctx: PSSL_CTX; flags: TIdC_ULONG): TIdC_ULONG;
  function SSL_dane_set_flags(ssl: PSSL; flags: TIdC_ULONG): TIdC_ULONG;
  function SSL_dane_clear_flags(ssl: PSSL; flags: TIdC_ULONG): TIdC_ULONG;

  function SSL_CTX_set1_param(ctx: PSSL_CTX; vpm: PX509_VERIFY_PARAM): TIdC_INT;
  function SSL_set1_param(ssl: PSSL; vpm: PX509_VERIFY_PARAM): TIdC_INT;

  function SSL_CTX_get0_param(ctx: PSSL_CTX): PX509_VERIFY_PARAM;
  function SSL_get0_param(ssl: PSSL): PX509_VERIFY_PARAM;

  function SSL_CTX_set_srp_username(ctx: PSSL_CTX; name: PIdAnsiChar): TIdC_INT;
  function SSL_CTX_set_srp_password(ctx: PSSL_CTX; password: PIdAnsiChar): TIdC_INT;
  function SSL_CTX_set_srp_strength(ctx: PSSL_CTX; strength: TIdC_INT): TIdC_INT;

  function SSL_CTX_set_srp_client_pwd_callback(ctx: PSSL_CTX; cb: SSL_CTX_set_srp_client_pwd_callback_cb): TIdC_INT;
  function SSL_CTX_set_srp_verify_param_callback(ctx: PSSL_CTX; cb: SSL_CTX_set_srp_verify_param_callback_cb): TIdC_INT;
  function SSL_CTX_set_srp_username_callback(ctx: PSSL_CTX; cb: SSL_CTX_set_srp_username_callback_cb): TIdC_INT;

  function SSL_CTX_set_srp_cb_arg(ctx: PSSL_CTX; arg: Pointer): TIdC_INT;
  function SSL_set_srp_server_param(s: PSSL; const N: PBIGNUm; const g: PBIGNUm; sa: PBIGNUm; v: PBIGNUm; info: PIdAnsiChar): TIdC_INT;
  function SSL_set_srp_server_param_pw(s: PSSL; const user: PIdAnsiChar; const pass: PIdAnsiChar; const grp: PIdAnsiChar): TIdC_INT;
  ...
  // */
  procedure SSL_CTX_set_client_hello_cb(c: PSSL_CTX; cb: SSL_client_hello_cb_fn; arg: Pointer);
  function SSL_client_hello_isv2(s: PSSL): TIdC_INT;
  function SSL_client_hello_get0_legacy_version(s: PSSL): TIdC_UINT;
  function SSL_client_hello_get0_random(s: PSSL; const &out: PPByte): TIdC_SIZET;
  function SSL_client_hello_get0_session_id(s: PSSL; const &out: PPByte): TIdC_SIZET;
  function SSL_client_hello_get0_ciphers(s: PSSL; const &out: PPByte): TIdC_SIZET;
  function SSL_client_hello_get0_compression_methods(s: PSSL; const &out: PPByte): TIdC_SIZET;
  function SSL_client_hello_get1_extensions_present(s: PSSL; &out: PPIdC_INT; outlen: PIdC_SIZET): TIdC_INT;
  function SSL_client_hello_get0_ext(s: PSSL; &type: TIdC_UINT; const &out: PPByte; outlen: PIdC_SIZET): TIdC_INT;
  procedure SSL_certs_clear(s: PSSL);
  procedure SSL_free(ssl: PSSL);

  (*
   * Windows application developer has to include windows.h to use these.
   *)
  function SSL_waiting_for_async(s: PSSL): TIdC_INT;
  function SSL_get_all_async_fds(s: PSSL; fds: POSSL_ASYNC_FD; numfds: PIdC_SIZET): TIdC_INT;
  function SSL_get_changed_async_fds(s: PSSL; addfd: POSSL_ASYNC_FD; numaddfds: PIdC_SIZET; delfd: POSSL_ASYNC_FD; numdelfds: PIdC_SIZET): TIdC_INT;
  function SSL_accept(ssl: PSSL): TIdC_INT;
  function SSL_stateless(s: PSSL): TIdC_INT;
  function SSL_connect(ssl: PSSL): TIdC_INT;
  function SSL_read(ssl: PSSL; buf: Pointer; num: TIdC_INT): TIdC_INT;
  ...
  function SSL_callback_ctrl(v1: PSSL; v2: TIdC_INT; v3: SSL_callback_ctrl_v3): TIdC_LONG;

  function SSL_ctrl(ssl: PSSL; cmd: TIdC_INT; larg: TIdC_LONG; parg: Pointer): TIdC_LONG;
  function SSL_CTX_ctrl(ctx: PSSL_CTX; cmd: TIdC_INT; larg: TIdC_LONG; parg: Pointer): TIdC_LONG;
  function SSL_CTX_callback_ctrl(v1: PSSL_CTX; v2: TIdC_INT; v3: SSL_CTX_callback_ctrl_v3): TIdC_LONG;

  function SSL_get_early_data_status(const s: PSSL): TIdC_INT;

  function SSL_get_error(const s: PSSL; ret_code: TIdC_INT): TIdC_INT;
  function SSL_get_version(const s: PSSL): PIdAnsiChar;

  (* This sets the 'default' SSL version that SSL_new() will create *)
  function SSL_CTX_set_ssl_version(ctx: PSSL_CTX; const meth: PSSL_METHOD): TIdC_INT;

  ///* Negotiate highest available SSL/TLS version */
  function TLS_method: PSSL_METHOD;
  ...
  function SSL_key_update(s: PSSL; updatetype: TIdC_INT): TIdC_INT;
  function SSL_get_key_update_type(const s: PSSL): TIdC_INT;
  function SSL_renegotiate(s: PSSL): TIdC_INT;
  function SSL_renegotiate_abbreviated(s: PSSL): TIdC_INT;
  function SSL_shutdown(s: PSSL): TIdC_INT;
  procedure SSL_CTX_set_post_handshake_auth(ctx: PSSL_CTX; val: TIdC_INT);
  procedure SSL_set_post_handshake_auth(s: PSSL; val: TIdC_INT);

  function SSL_renegotiate_pending(const s: PSSL): TIdC_INT;
  function SSL_verify_client_post_handshake(s: PSSL): TIdC_INT;

  function SSL_CTX_get_ssl_method(const ctx: PSSL_CTX): PSSL_METHOD;
  function SSL_get_ssl_method(const s: PSSL): PSSL_METHOD;
  function SSL_set_ssl_method(s: PSSL; const method: PSSL_METHOD): TIdC_INT;
  function SSL_alert_type_string_long(value: TIdC_INT): PIdAnsiChar;
  function SSL_alert_type_string(value: TIdC_INT): PIdAnsiChar;
  function SSL_alert_desc_string_long(value: TIdC_INT): PIdAnsiChar;
  function SSL_alert_desc_string(value: TIdC_INT): PIdAnsiChar;
  ...
  function SSL_add_client_CA(ssl: PSSL; x: PX509): TIdC_INT;
  function SSL_CTX_add_client_CA(ctx: PSSL_CTX; x: PX509): TIdC_INT;
  ...
  function SSL_dup(ssl: PSSL): PSSL;

  function SSL_get_certificate(const ssl: PSSL): PX509;
  (*
   * EVP_PKEY
   *)
  function SSL_get_privatekey(const ssl: PSSL): PEVP_PKEY;

  function SSL_CTX_get0_certificate(const ctx: PSSL_CTX): PX509;
  function SSL_CTX_get0_privatekey(const ctx: PSSL_CTX): PEVP_PKEY;

  procedure SSL_CTX_set_quiet_shutdown(ctx: PSSL_CTX; mode: TIdC_INT);
  function SSL_CTX_get_quiet_shutdown(const ctx: PSSL_CTX): TIdC_INT;
  procedure SSL_set_quiet_shutdown(ssl: PSSL; mode: TIdC_INT);
  function SSL_get_quiet_shutdown(const ssl: PSSL): TIdC_INT;
  procedure SSL_set_shutdown(ssl: PSSL; mode: TIdC_INT);
  function SSL_get_shutdown(const ssl: PSSL): TIdC_INT;
  function SSL_version(const ssl: PSSL): TIdC_INT;
  function SSL_client_version(const s: PSSL): TIdC_INT;
  function SSL_CTX_set_default_verify_paths(ctx: PSSL_CTX): TIdC_INT;
  function SSL_CTX_set_default_verify_dir(ctx: PSSL_CTX): TIdC_INT;
  function SSL_CTX_set_default_verify_file(ctx: PSSL_CTX): TIdC_INT;
  function SSL_CTX_load_verify_locations(ctx: PSSL_CTX; const CAfile: PIdAnsiChar; const CApath: PIdAnsiChar): TIdC_INT;
  //# define SSL_get0_session SSL_get_session/* just peek at pointer */
  function SSL_get_session(const ssl: PSSL): PSSL_SESSION;
  (* obtain a reference count *)
  function SSL_get1_session(ssl: PSSL): PSSL_SESSION;
  function SSL_get_SSL_CTX(const ssl: PSSL): PSSL_CTX;
  function SSL_set_SSL_CTX(ssl: PSSL; ctx: PSSL_CTX): PSSL_CTX;
  procedure SSL_set_info_callback(ssl: PSSL; cb: SSL_info_callback);
  function SSL_get_info_callback(const ssl: PSSL): SSL_info_callback;
  function SSL_get_state(const ssl: PSSL): OSSL_HANDSHAKE_STATE;

  procedure SSL_set_verify_result(ssl: PSSL; v: TIdC_LONG);
  function SSL_get_verify_result(const ssl: PSSL): TIdC_LONG;
  //__owur STACK_OF(X509) *SSL_get0_verified_chain(const s: PSSL);

  function SSL_get_client_random(const ssl: PSSL; &out: PByte; outlen: TIdC_SIZET): TIdC_SIZET;
  function SSL_get_server_random(const ssl: PSSL; &out: PByte; outlen: TIdC_SIZET): TIdC_SIZET;
  function SSL_SESSION_get_master_key(const sess: PSSL_SESSION; &out: PByte; outlen: TIdC_SIZET): TIdC_SIZET;
  function SSL_SESSION_set1_master_key(sess: PSSL_SESSION; const &in: PByte; len: TIdC_SIZET): TIdC_INT;
  function SSL_SESSION_get_max_fragment_length(const sess: PSSL_SESSION): TIdC_UINT8;

  //#define SSL_get_ex_new_index(l, p, newf, dupf, freef) \
  //    CRYPTO_get_ex_new_index(CRYPTO_EX_INDEX_SSL, l, p, newf, dupf, freef)
  function SSL_set_ex_data(ssl: PSSL; idx: TIdC_INT; data: Pointer): TIdC_INT;
  function SSL_get_ex_data(const ssl: PSSL; idx: TIdC_INT): Pointer;
  //#define SSL_SESSION_get_ex_new_index(l, p, newf, dupf, freef) \
  //    CRYPTO_get_ex_new_index(CRYPTO_EX_INDEX_SSL_SESSION, l, p, newf, dupf, freef)
  function SSL_SESSION_set_ex_data(ss: PSSL_SESSION; idx: TIdC_INT; data: Pointer): TIdC_INT;
  function SSL_SESSION_get_ex_data(const ss: PSSL_SESSION; idx: TIdC_INT): Pointer;
  ...
  procedure SSL_CTX_set_default_read_buffer_len(ctx: PSSL_CTX; len: TIdC_SIZET);
  procedure SSL_set_default_read_buffer_len(s: PSSL; len: TIdC_SIZET);

  procedure SSL_CTX_set_tmp_dh_callback(ctx: PSSL_CTX; dh: SSL_CTX_set_tmp_dh_callback_dh);
  procedure SSL_set_tmp_dh_callback(ssl: PSSL; dh: SSL_set_tmp_dh_callback_dh);
  ...
  function SSL_CIPHER_find(ssl: PSSL; const ptr: PByte): PSSL_CIPHER;
  function SSL_CIPHER_get_cipher_nid(const c: PSSL_CIPHEr): TIdC_INT;
  function SSL_CIPHER_get_digest_nid(const c: PSSL_CIPHEr): TIdC_INT;
  ...
  function SSL_set_session_ticket_ext(s: PSSL; ext_data: Pointer; ext_len: TIdC_INT): TIdC_INT;
  //
  function SSL_set_session_ticket_ext_cb(s: PSSL; cb: tls_session_ticket_ext_cb_fn; arg: Pointer): TIdC_INT;
  ...
  procedure SSL_CTX_set_not_resumable_session_callback(ctx: PSSL_CTX; cb: SSL_CTX_set_not_resumable_session_callback_cb);
  procedure SSL_set_not_resumable_session_callback(ssl: PSSL; cb: SSL_set_not_resumable_session_callback_cb);
  procedure SSL_CTX_set_record_padding_callback(ctx: PSSL_CTX; cb: SSL_CTX_set_record_padding_callback_cb);

  procedure SSL_CTX_set_record_padding_callback_arg(ctx: PSSL_CTX; arg: Pointer);
  function SSL_CTX_get_record_padding_callback_arg(const ctx: PSSL_CTX): Pointer;
  function SSL_CTX_set_block_padding(ctx: PSSL_CTX; block_size: TIdC_SIZET): TIdC_INT;

  procedure SSL_set_record_padding_callback(ssl: PSSL; cb: SSL_set_record_padding_callback_cb);

  procedure SSL_set_record_padding_callback_arg(ssl: PSSL; arg: Pointer);
  function SSL_get_record_padding_callback_arg(const ssl: PSSL): Pointer;
  function SSL_set_block_padding(ssl: PSSL; block_size: TIdC_SIZET): TIdC_INT;
  function SSL_set_num_tickets(s: PSSL; num_tickets: TIdC_SIZET): TIdC_INT;
  function SSL_get_num_tickets(const s: PSSL): TIdC_SIZET;
  function SSL_CTX_set_num_tickets(ctx: PSSL_CTX; num_tickets: TIdC_SIZET): TIdC_INT;
  function SSL_CTX_get_num_tickets(const ctx: PSSL_CTX): TIdC_SIZET;

  //# if OPENSSL_API_COMPAT < 0x10100000L
  //#  define SSL_cache_hit(s) SSL_session_reused(s)
  //# endif

  function SSL_session_reused(const s: PSSL): TIdC_INT;
  function SSL_is_server(const s: PSSL): TIdC_INT;

  function SSL_CONF_CTX_new: PSSL_CONF_CTX;
  function SSL_CONF_CTX_finish(cctx: PSSL_CONF_CTX): TIdC_INT;
  procedure SSL_CONF_CTX_free(cctx: PSSL_CONF_CTX);
  function SSL_CONF_CTX_set_flags(cctx: PSSL_CONF_CTX; flags: TIdC_UINT): TIdC_UINT;
  function SSL_CONF_CTX_clear_flags(cctx: PSSL_CONF_CTX; flags: TIdC_UINT): TIdC_UINT;
  function SSL_CONF_CTX_set1_prefix(cctx: PSSL_CONF_CTX; const pre: PIdAnsiChar): TIdC_INT;
  function SSL_CONF_cmd(cctx: PSSL_CONF_CTX; const cmd: PIdAnsiChar; const value: PIdAnsiChar): TIdC_INT;
  function SSL_CONF_cmd_argv(cctx: PSSL_CONF_CTX; pargc: PIdC_INT; pargv: PPPIdAnsiChar): TIdC_INT;
  function SSL_CONF_cmd_value_type(cctx: PSSL_CONF_CTX; const cmd: PIdAnsiChar): TIdC_INT;

  procedure SSL_CONF_CTX_set_ssl(cctx: PSSL_CONF_CTX; ssl: PSSL);
  procedure SSL_CONF_CTX_set_ssl_ctx(cctx: PSSL_CONF_CTX; ctx: PSSL_CTX);
  procedure SSL_add_ssl_module;
  function SSL_config(s: PSSL; const name: PIdAnsiChar): TIdC_INT;
  function SSL_CTX_config(ctx: PSSL_CTX; const name: PIdAnsiChar): TIdC_INT;
  ...
  function DTLSv1_listen(s: PSSL; client: PBIO_ADDr): TIdC_INT;
  ...
  // */
  function SSL_enable_ct(s: PSSL; validation_mode: TIdC_INT): TIdC_INT;
  function SSL_CTX_enable_ct(ctx: PSSL_CTX; validation_mode: TIdC_INT): TIdC_INT;

  ///*
  // * Report whether a non-NULL callback is enabled.
  // */
  function SSL_ct_is_enabled(const s: PSSL): TIdC_INT;
  function SSL_CTX_ct_is_enabled(const ctx: PSSL_CTX): TIdC_INT;

  ///* Gets the SCTs received from a connection */
  //const STACK_OF(SCT) *SSL_get0_peer_scts(s: PSSL);

  function SSL_CTX_set_default_ctlog_list_file(ctx: PSSL_CTX): TIdC_INT;
  function SSL_CTX_set_ctlog_list_file(ctx: PSSL_CTX; const path: PIdAnsiChar): TIdC_INT;
  procedure SSL_CTX_set0_ctlog_store(ctx: PSSL_CTX; logs: PCTLOG_STORE);

  // const CTLOG_STORE *SSL_CTX_get0_ctlog_store(const ctx: PSSL_CTX);

  // # endif /* OPENSSL_NO_CT */

  procedure SSL_set_security_level(s: PSSL; level: TIdC_INT);

  ////__owur TIdC_INT SSL_get_security_level(const s: PSSL);
  procedure SSL_set_security_callback(s: PSSL; cb: SSL_security_callback);
  function SSL_get_security_callback(const s: PSSL): SSL_security_callback;
  procedure SSL_set0_security_ex_data(s: PSSL; ex: Pointer);
  function SSL_get0_security_ex_data(const s: PSSL): Pointer;
  procedure SSL_CTX_set_security_level(ctx: PSSL_CTX; level: TIdC_INT);
  function SSL_CTX_get_security_level(const ctx: PSSL_CTX): TIdC_INT;
  ...
  function SSL_CTX_get0_security_ex_data(const ctx: PSSL_CTX): Pointer;

  procedure SSL_CTX_set0_security_ex_data(ctx: PSSL_CTX; ex: Pointer);

  function OPENSSL_init_ssl(opts: TIdC_UINT64; const settings: POPENSSL_INIT_SETTINGS): TIdC_INT;

  //# ifndef OPENSSL_NO_UNIT_TEST
  //__owur const struct openssl_ssl_test_functions *SSL_test_functions(void);
  //# endif

  function SSL_free_buffers(ssl: PSSL): TIdC_INT;
  function SSL_alloc_buffers(ssl: PSSL): TIdC_INT;

  function SSL_CTX_set_session_ticket_cb(ctx: PSSL_CTX; gen_cb: SSL_CTX_generate_session_ticket_fn; dec_cb: SSL_CTX_decrypt_session_ticket_fn; arg: Pointer): TIdC_INT;

  function SSL_SESSION_set1_ticket_appdata(ss: PSSL_SESSION; const data: Pointer; len: TIdC_SIZET): TIdC_INT;
  function SSL_SESSION_get0_ticket_appdata(ss: PSSL_SESSION; data: PPointer; len: PIdC_SIZET): TIdC_INT;

  //extern const PIdAnsiChar SSL_version_str[];

  procedure DTLS_set_timer_cb(s: PSSL; cb: DTLS_timer_cb);
  procedure SSL_CTX_set_allow_early_data_cb(ctx: PSSL_CTX; cb: SSL_allow_early_data_cb_fN; arg: Pointer);
  procedure SSL_set_allow_early_data_cb(s: PSSL; cb: SSL_allow_early_data_cb_fN; arg: Pointer);
