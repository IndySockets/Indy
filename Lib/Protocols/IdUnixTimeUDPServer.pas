...
uses
  ...
  Classes,
  {$ENDIF}
  IdAssignedNumbers, IdUDPServer, IdTimeUDPServer;

{
  ...
  Unix is an operating system developed in 1969 by Ken Thompson.
  UnixTime counts "epochs" or seconds since the Year 1970. UnixTime
  recently hit it's billionth birthday.

  Because Unix is widely used in many environments, UnixTime was developed
  into a loose simple time protocol in the late 80's and early 90's. No
  formal UnixTime protocol has ever been officially published as an internet
  protocol - until now.
...
type
   TIdUnixTimeUDPServer = class(TIdCustomTimeUDPServer)
   protected
...
{$IFDEF WORKAROUND_INLINE_CONSTRUCTORS}
constructor TIdUnixTimeUDPServer.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
end;
{$ENDIF}

procedure TIdUnixTimeUDPServer.InitComponent;
begin
  inherited;
