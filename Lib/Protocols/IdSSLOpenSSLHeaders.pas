...
//					HMAC_CTX *hctx, int enc);
  Ptlsext_ticket_key_cb = function (ssl : PSSL; name, iv : PChar;
...
{$IFNDEF OPENSSL_NO_TLSEXT}
function IdSslSetTlsExtHostName(s : PSSL; name : String) : TIdC_LONG;
...
function IdSslSetTlsExtHostName(s : PSSL; name : String) : TIdC_LONG;
 {$IFDEF USEINLINE} inline; {$ENDIF}
begin
  Result := IdSslCtrl(ssl, OPENSSL_SSL_CTRL_SET_TLSEXT_HOSTNAME, OPENSSL_TLSEXT_NAMETYPE_host_name,PChar(name));
end;
...
function IdSslSetTlsExtStatusType(ssl : PSSL; _type : TIdC_LONG):TIdC_LONG;
 {$IFDEF USEINLINE} inline; {$ENDIF}
begin
  Result := IdSslCtrl(ssl, OPENSSL_SSL_CTRL_SET_TLSEXT_STATUS_REQ_TYP,_type,nil);
end;
...
function IdSslSetTlsExtStatusIds(ssl : PSSL; arg : Pointer) : TIdC_LONG;
 {$IFDEF USEINLINE} inline; {$ENDIF}
begin
  Result := IdSslCtrl(ssl, OPENSSL_CTRL_SET_TLSEXT_STATUS_REQ_IDS,0,arg);
end;
...
function IdSslSslCtxSetTlsExtServerNameCallback(ctx : PSSL_CTX; cb :SSL_callback_ctrl_fp):TIdC_LONG;
begin
  Result := IdSslCtxCallbackCtrl(ssl, OPENSSL_SSL_CTRL_SET_TLSEXT_SERVERNAME_CB, cb);
end;
