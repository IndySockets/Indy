...
{$ELSE}
  function ERR_load_CRYPTO_strings: TIdC_INT cdecl; external {$IFNDEF OPENSSL_USE_STATIC_LIBRARY}CLibCrypto{$ENDIF}{$IFDEF OPENSSL_USE_DELAYED_LOADING} delayed{$ENDIF};
...
begin
  ERR_load_CRYPTO_strings := LoadFunction('ERR_load_CRYPTO_strings',AFailed);
end;
