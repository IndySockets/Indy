...
type
  ...
  {$ELSE}
    {$IFNDEF NO_REDECLARE}
 // TCriticalSection = {$IFDEF USE_UNIT_SCOPE_NAMES}System.SyncObjs{$ELSE}SyncObjs{$ENDIF}.TCriticalSection;
    {$ENDIF}
  {$ENDIF}
...
const
  ...
  GOSType = otWindows;
  GPathDelim = '\'; {do not localize}
  Infinite = {$IFDEF USE_UNIT_SCOPE_NAMES}Winapi.Windows{$ELSE}Windows{$ENDIF}.INFINITE; { redeclare here for use elsewhere without using Windows.pas }  // cls modified 1/23/2002
  {$ENDIF}
...
begin
  ...
  Result := {$IFDEF WINDOWS}{$IFDEF USE_UNIT_SCOPE_NAMES}Winapi.Windows{$ELSE}Windows{$ENDIF}.{$ENDIF}GetProcAddress(ALibHandle, PIdLibFuncNameChar(AProcName));
...
begin
  ...
  Result := UInt64({$IFDEF USE_UNIT_SCOPE_NAMES}Winapi.Windows{$ELSE}Windows{$ENDIF}.GetTickCount);
end;
...
begin
  ...
  {$IFDEF USE_HI_PERF_COUNTER_FOR_TICKS}
    {$IFDEF WINCE}
  if {$IFDEF USE_UNIT_SCOPE_NAMES}Winapi.Windows{$ELSE}Windows{$ENDIF}.QueryPerformanceCounter(@nTime) then begin
    if {$IFDEF USE_UNIT_SCOPE_NAMES}Winapi.Windows{$ELSE}Windows{$ENDIF}.QueryPerformanceFrequency(@freq) then begin
...
{$IFDEF USE_INLINE}inline;{$ENDIF}
begin
  Result := {$IFDEF USE_UNIT_SCOPE_NAMES}System.SysUtils{$ELSE}SysUtils{$ENDIF}.StrScan(Str, Chr);
end;
{$ENDIF}
...
{$IFDEF USE_INLINE}inline;{$ENDIF}
begin
  Result := {$IFDEF USE_UNIT_SCOPE_NAMES}System.SysUtils{$ELSE}SysUtils{$ENDIF}.AnsiPos(Substr, S);
end;

function InternalAnsiStrScan(Str: PChar; Chr: Char): PChar;
{$IFDEF USE_INLINE}inline;{$ENDIF}
begin
  Result := {$IFDEF USE_UNIT_SCOPE_NAMES}System.SysUtils{$ELSE}SysUtils{$ENDIF}.AnsiStrScan(Str, Chr);
end;
{$ENDIF}
...
begin
  ...
  {$IFDEF USE_UNIT_SCOPE_NAMES}Winapi.Windows{$ELSE}Windows{$ENDIF}.Sleep(ATime);
...
begin
  ...
  Result := {$IFDEF USE_UNIT_SCOPE_NAMES}System.SysUtils{$ELSE}SysUtils{$ENDIF}.Format(AFormat, Args);
  {$ELSE}
    {$IFDEF HAS_TFormatSettings}
  ...
  begin
    Len := {$IFDEF USE_UNIT_SCOPE_NAMES}System.SysUtils{$ELSE}SysUtils{$ENDIF}.FormatBuf(Buffer, Length(Buffer) - 1, Pointer(AFormat)^,
  ...
  begin
    ...
    begin
      ...
      Len := {$IFDEF USE_UNIT_SCOPE_NAMES}System.SysUtils{$ELSE}SysUtils{$ENDIF}.FormatBuf(PChar(Result), BufLen - 1, Pointer(AFormat)^,
        Length(AFormat), Args, EnglishFmt);
    end;
    SetLength(Result, Len);
  end else
  begin
    ...
    {$ENDIF}
      ...
      {$ELSE}
  Result := {$IFDEF USE_UNIT_SCOPE_NAMES}System.SysUtils{$ELSE}SysUtils{$ENDIF}.Format(AFormat, Args, EnglishFmt);
      {$ENDIF}
    {$ELSE}
...
begin
  ...
  Result := {$IFDEF USE_UNIT_SCOPE_NAMES}System.SysUtils{$ELSE}SysUtils{$ENDIF}.IncludeTrailingPathDelimiter(S);
  {$ELSE}
  Result := {$IFDEF USE_UNIT_SCOPE_NAMES}System.SysUtils{$ELSE}SysUtils{$ENDIF}.IncludeTrailingBackslash(S);
  {$ENDIF}
end;
...
begin
  {$IFDEF HAS_SysUtils_IncludeExcludeTrailingPathDelimiter}
  Result := {$IFDEF USE_UNIT_SCOPE_NAMES}System.SysUtils{$ELSE}SysUtils{$ENDIF}.ExcludeTrailingPathDelimiter(S);
  {$ELSE}
  Result := {$IFDEF USE_UNIT_SCOPE_NAMES}System.SysUtils{$ELSE}SysUtils{$ENDIF}.ExcludeTrailingBackslash(S);
  {$ENDIF}
end;
...
begin
  ...
  if Length(NewPattern) = PatLen then begin
    ...
    Result := {$IFDEF USE_UNIT_SCOPE_NAMES}System.SysUtils{$ELSE}SysUtils{$ENDIF}.StringReplace(S, OldPattern, NewPattern, [rfReplaceAll]);
  end;
end;
...
{$IFDEF USE_INLINE}inline;{$ENDIF}
begin
  Result := {$IFDEF USE_UNIT_SCOPE_NAMES}System.SysUtils{$ELSE}SysUtils{$ENDIF}.StringReplace(s, OldPattern, NewPattern, []);
end;
...
begin
  {$IFDEF HAS_UNIT_DateUtils}
  Result := {$IFDEF USE_UNIT_SCOPE_NAMES}System.DateUtils{$ELSE}DateUtils{$ENDIF}.IncMilliSecond(ADateTime, AMSec);
  {$ELSE}
...
begin
  ...
  {$ELSE}
  LAge := {$IFDEF USE_UNIT_SCOPE_NAMES}System.SysUtils{$ELSE}SysUtils{$ENDIF}.FileAge(AFileName);
...
begin
  {$IFDEF HAS_SysUtils_DirectoryExists}
  Result := {$IFDEF USE_UNIT_SCOPE_NAMES}System.SysUtils{$ELSE}SysUtils{$ENDIF}.DirectoryExists(ADirectory);
  {$ELSE}
...
{$IFDEF USE_INLINE}inline;{$ENDIF}
begin
  Result := {$IFDEF USE_UNIT_SCOPE_NAMES}System.SysUtils{$ELSE}SysUtils{$ENDIF}.StrToInt64Def(Trim(S), ADefault);
end;

function IndyStrToInt64(const S: string): Int64;
{$IFDEF USE_INLINE}inline;{$ENDIF}
begin
  Result := {$IFDEF USE_UNIT_SCOPE_NAMES}System.SysUtils{$ELSE}SysUtils{$ENDIF}.StrToInt64(Trim(S));
end;
...
begin
  {$IFDEF WINCE}
  Result := {$IFDEF USE_UNIT_SCOPE_NAMES}System.SysUtils{$ELSE}SysUtils{$ENDIF}.WinCEMajorVersion;
  {$ELSE}
  Result := {$IFDEF USE_UNIT_SCOPE_NAMES}System.SysUtils{$ELSE}SysUtils{$ENDIF}.Win32MajorVersion;
  {$ENDIF}
end;
...
begin
  {$IFDEF WINCE}
  Result := {$IFDEF USE_UNIT_SCOPE_NAMES}System.SysUtils{$ELSE}SysUtils{$ENDIF}.WinCEMinorVersion;
  {$ELSE}
  Result := {$IFDEF USE_UNIT_SCOPE_NAMES}System.SysUtils{$ELSE}SysUtils{$ENDIF}.Win32MinorVersion;
  {$ENDIF}
end;
...
begin
  ...
  {$IFDEF WINCE}
  Result := {$IFDEF USE_UNIT_SCOPE_NAMES}System.SysUtils{$ELSE}SysUtils{$ENDIF}.WinCEBuildNumber and $FFFF;
  {$ELSE}
  Result := {$IFDEF USE_UNIT_SCOPE_NAMES}System.SysUtils{$ELSE}SysUtils{$ENDIF}.Win32BuildNumber and $FFFF;
  {$ENDIF}
end;
...
begin
  {$IFDEF WINCE}
  Result := {$IFDEF USE_UNIT_SCOPE_NAMES}System.SysUtils{$ELSE}SysUtils{$ENDIF}.WinCEPlatform;
  {$ELSE}
  Result := {$IFDEF USE_UNIT_SCOPE_NAMES}System.SysUtils{$ELSE}SysUtils{$ENDIF}.Win32Platform;
  {$ENDIF}
end;
